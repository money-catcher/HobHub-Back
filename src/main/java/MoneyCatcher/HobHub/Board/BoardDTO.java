...
public class BoardDTO {
    ...
    public static BoardDTO toBoardDTO(BoardEntity boardEntity)
    {
        ...
        {
            boardDTO.setFileAttached(boardEntity.getFileAttached());//파일이 없으면 FileAttahced는 그대로 0으로 반납
        }
        else {
            //위에 내용이랑 비슷함 엔티티에서 가져와서 디티오로 리턴하기
            boardDTO.setFileAttached(boardEntity.getFileAttached());//파일있으면 1로 셋팅해서 반납
