...
public class FileService {
    ...
    public String uploadImageToS3(MultipartFile image) throws IOException { //이미지를 S3에 업로드하고 이미지의 url을 반환
        ...
        try {
            ...
            ).withCannedAcl(CannedAccessControlList.PublicRead)); //s3에 저장

        } catch (IOException e) {
            throw new IOException();
        }
        return amazonS3.getUrl(bucketName, changedName).toString(); //서버 url 리턴
    }
    public FileEntity uploadImage(MultipartFile image) throws IOException {
        String originName = image.getOriginalFilename();
        String storedImagePath = uploadImageToS3(image);

        FileEntity newImage = FileEntity.builder() //이미지에 대한 정보를 담아서 반환
                .originalFileName(originName)
                .storedFileName(storedImagePath)
                .build();
        return newImage;
    }
}
